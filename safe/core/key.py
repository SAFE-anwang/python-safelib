...
import safe
import safe.signature

import safe.core.script
...
class CECKey:
    ...
    def sign(self, hash): # pylint: disable=redefined-builtin
        ...
        assert 1 == result
        if safe.core.script.IsLowDERSignature(mb_sig.raw[:sig_size0.value]):
            return mb_sig.raw[:sig_size0.value]
        else:
    ...
    def sign_compact(self, hash): # pylint: disable=redefined-builtin
        ...
        sig = safe.signature.DERSignature.deserialize(sig)
