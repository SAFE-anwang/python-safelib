...
import safe.bech32
import safe.core
import safe.core.key
import safe.core.script as script
...
class CBitcoinAddress(object):
    ...
    def __new__(cls, s):
        ...
        except safe.bech32.Bech32Error:
            pass

        try:
            return CBase58BitcoinAddress(s)
        except safe.base58.Base58Error:
            pass
...
class CBech32BitcoinAddress(safe.bech32.CBech32Data, CBitcoinAddress):
...
class CBase58BitcoinAddress(safe.base58.CBase58Data, CBitcoinAddress):
    ...
    def from_bytes(cls, data, nVersion):
        ...
        if nVersion == safe.params.BASE58_PREFIXES['SCRIPT_ADDR']:
            self.__class__ = P2SHBitcoinAddress

        elif nVersion == safe.params.BASE58_PREFIXES['PUBKEY_ADDR']:
...
class P2SHBitcoinAddress(CBase58BitcoinAddress):
    ...
    def from_bytes(cls, data, nVersion=None):
        if nVersion is None:
            nVersion = safe.params.BASE58_PREFIXES['SCRIPT_ADDR']

        elif nVersion != safe.params.BASE58_PREFIXES['SCRIPT_ADDR']:
    ...
    def from_scriptPubKey(cls, scriptPubKey):
        ...
        if scriptPubKey.is_p2sh():
            return cls.from_bytes(scriptPubKey[2:22], safe.params.BASE58_PREFIXES['SCRIPT_ADDR'])
    ...
    def to_scriptPubKey(self):
        ...
        assert self.nVersion == safe.params.BASE58_PREFIXES['SCRIPT_ADDR']
...
class P2PKHBitcoinAddress(CBase58BitcoinAddress):
    ...
    def from_bytes(cls, data, nVersion=None):
        if nVersion is None:
            nVersion = safe.params.BASE58_PREFIXES['PUBKEY_ADDR']
    ...
    def from_pubkey(cls, pubkey, accept_invalid=False):
        ...
        if not accept_invalid:
            if not isinstance(pubkey, safe.core.key.CPubKey):
                pubkey = safe.core.key.CPubKey(pubkey)
            if not pubkey.is_fullyvalid:
                raise CBitcoinAddressError('invalid pubkey')

        pubkey_hash = safe.core.Hash160(pubkey)
    ...
    def from_scriptPubKey(cls, scriptPubKey, accept_non_canonical_pushdata=True, accept_bare_checksig=True):
        ...
        if accept_non_canonical_pushdata:
            ...
            except safe.core.script.CScriptInvalidError:
                raise CBitcoinAddressError('not a P2PKH scriptPubKey: script is invalid')

        if scriptPubKey.is_witness_v0_keyhash():
            return cls.from_bytes(scriptPubKey[2:22], safe.params.BASE58_PREFIXES['PUBKEY_ADDR'])
        elif scriptPubKey.is_witness_v0_nested_keyhash():
            return cls.from_bytes(scriptPubKey[3:23], safe.params.BASE58_PREFIXES['PUBKEY_ADDR'])
    ...
    def to_scriptPubKey(self, nested=False):
        """Convert an address to a scriptPubKey"""
        assert self.nVersion == safe.params.BASE58_PREFIXES['PUBKEY_ADDR']
...
class CKey(object):
    ...
    def __init__(self, secret, compressed=True):
        self._cec_key = safe.core.key.CECKey()
        self._cec_key.set_secretbytes(secret)
        self._cec_key.set_compressed(compressed)

        self.pub = safe.core.key.CPubKey(self._cec_key.get_pubkey(), self._cec_key)
...
class CBitcoinSecretError(safe.base58.Base58Error):
    pass

class CBitcoinSecret(safe.base58.CBase58Data, CKey):
    ...
    def from_secret_bytes(cls, secret, compressed=True):
        ...
        self = cls.from_bytes(secret + (b'\x01' if compressed else b''),
                              safe.params.BASE58_PREFIXES['SECRET_KEY'])
        self.__init__(None)
        return self

    def __init__(self, s):
        if self.nVersion != safe.params.BASE58_PREFIXES['SECRET_KEY']:
            raise CBitcoinSecretError('Not a base58-encoded secret key: got nVersion=%d; expected nVersion=%d' % \
                                      (self.nVersion, safe.params.BASE58_PREFIXES['SECRET_KEY']))
