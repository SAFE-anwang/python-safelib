...
from safe.core.key import CPubKey
from safe.core.serialize import ImmutableSerializable
from safe.wallet import P2PKHBitcoinAddress
import safe
import base64
...
class BitcoinMessage(ImmutableSerializable):
    ...
    def stream_deserialize(cls, f):
        magic = safe.core.serialize.BytesSerializer.stream_deserialize(f)
        message = safe.core.serialize.BytesSerializer.stream_deserialize(f)
        return cls(message, magic)

    def stream_serialize(self, f):
        safe.core.serialize.BytesSerializer.stream_serialize(self.magic, f)
        safe.core.serialize.BytesSerializer.stream_serialize(self.message, f)
