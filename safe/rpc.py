...
import safe
from safe.core import COIN, x, lx, b2lx, CBlock, CBlockHeader, CTransaction, COutPoint, CTxOut
from safe.core.script import CScript
from safe.wallet import CBitcoinAddress, CBitcoinSecret
...
class BaseProxy(object):
    ...
    def __init__(self,
        ...
        if service_url is None:
            # Figure out the path to the safe.conf file
            if btc_conf_file is None:
                ...
                btc_conf_file = os.path.join(btc_conf_file, 'safe.conf')

            # Bitcoin Core accepts empty rpcuser, not specified in btc_conf_file
            conf = {'rpcuser': ""}

            # Extract contents of safe.conf to build service_url
            try:
            ...
            # Treat a missing safe.conf as though it were empty
            except FileNotFoundError:
                pass

            if service_port is None:
                service_port = safe.params.RPC_PORT
            ...
            if safe.params.NAME != "mainnet":
                cookie_dir = os.path.join(cookie_dir, safe.params.NAME)
...
class RawProxy(BaseProxy):
    ...
    def __getattr__(self, name):
        ...
        # Make debuggers show <function safe.rpc.name> rather than <function
        # safe.rpc.<lambda>>
        f.__name__ = name
        return f
...
class Proxy(BaseProxy):
    ...
    Unlike ``RawProxy``, data is passed as ``safe.core`` objects or packed
    ...
    def __init__(self,
        ...
        specified, ``~/.bitcoin/safe.conf`` or equivalent is used by
    ...
    def submitblock(self, block, params=None):
        ...
        https://en.safe.it/wiki/BIP_0022 for full specification.
        """
